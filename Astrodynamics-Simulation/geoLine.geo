#version 330 core
    ...
    uniform vec2 u_viewport_size; // layout(location=1)
    uniform vec2 u_aa_radius; // layout(location=2)

    in vec4 v_col[];
    noperspective in float v_line_width[]; // had noperspective

    out vec4 g_col;
    noperspective out float g_line_width; // here and three below out's had noperspective
