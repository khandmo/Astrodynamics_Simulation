...
Mesh System::initBody(const char* name, const char* texFilePath, float radius, float outerRadius, float axialTilt, float angleOfRot,  bool isLight, bool areRings, int soiID, int baryID, int spiceID, int orbPeriod) {
	// init texture
	...
	SystemTime();
	Mesh body(name, obj.vertices, obj.indices, objTex, isLight, areRings, &shader, baryID, spiceID, sysTime.time_in_sec, orbPeriod); // velocity will be updated with SPICE integration
